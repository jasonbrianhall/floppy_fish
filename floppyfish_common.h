...
// floppyfish_antarctic.cpp) includes this and implements the same
// five-function contract below; floppyfish_common.cpp holds the small
// dispatchers that are the only code allowed to know all themes exist, and
// floppyfish.cpp (the core) drives state/physics/UI and calls into the
// dispatchers.
...
typedef enum {
    ...
    FF_THEME_RAINBOW = 4, FF_THEME_DINO = 5, FF_THEME_ANTARCTIC = 6, FF_THEME_COUNT = 7
} FFTheme;
